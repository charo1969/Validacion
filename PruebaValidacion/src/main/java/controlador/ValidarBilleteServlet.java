...
public class ValidarBilleteServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		boolean error = false;
		/**
		 * recogemos los parámetros y comprobamos los parámetros
		 */
		...
		try {
			...
			int tipoBillete = Integer.parseInt(request.getParameter("tipoBillete"));
			...
			if (tipoBillete == 2 && edad>26) {
				...
				session.setAttribute("errorEdad",
						"Debe tener mas de 65 para usar el billete 3º edad");
				error = true;
				
			}
		
		} catch(NumberFormatException e) {
			session.setAttribute("errorEdad", "Debe introducir un valor numérico");
			error = true;
		}
		...
		try {
			...
			session.setAttribute("errorPasajeros","Debe introducir un numero válido" );
			error = true;
